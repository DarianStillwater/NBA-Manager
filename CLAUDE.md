...
| **ArtInjector** | `UI/ArtInjector.cs` | Registers game panels on Start(). Handles PreGame/PostGame flows. |
...
| `Core/Data/PlayerDatabase.cs` | ~300 | Loads from `StreamingAssets/Data/players.json`. Preprocesses Position enums and BirthDate strings. |
...
- Save/Load with auto-save rotation and Ironman mode
- FM-style dashboard with live date/record updates
- Full roster with player detail (bio, stats, scouting, contract tabs)
- Schedule view with game detail
- Conference standings with playoff line
- Game simulation with box scores
- Advance Day / Sim to Game with pre-game screen
